...
@click.command(
    "clear",
    help=f"Delete all backups in a {palette.sky}'BACKUP_SPACE'{RESET} identified "
...
def clear(backup_space: str, force: bool, verbose: bool, debug: bool) -> None:
    ...
    if not force:
        confirm = ConfirmInput(
            message=f"> Are you sure you want to clear the backup space "
    ...
    if confirm:
        try:
            space.clear(verbosity_level=verbose)
        ...
        print(f"{palette.maroon}Clearing canceled.{RESET}")
